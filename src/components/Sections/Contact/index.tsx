...
import {contactItems, SectionId} from '../../../data/data';
    ...
    <Section className="bg-neutral-800" sectionId={SectionId.Contact}>
      <div className="flex flex-col gap-y-6">
        <div className="flex flex-col gap-6 md:flex-row md:items-center">
          ...
          <h2 className="text-2xl font-bold text-white">{strings.contact.headerText}</h2>
        </div>
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div className="order-1 col-span-1 flex flex-col gap-y-4 md:order-2">
            <p className="prose leading-6 text-neutral-300">{strings.contact.description}</p>
            <dl className="flex flex-col space-y-4 text-base text-neutral-500 sm:space-y-2">
              {contactItems.map(({type, href}, index) => {
                const text = strings.contact.items[index].text;
